 ...
 * migrate-to-turso.js
 * -----------------------------------------------------------------
 * Lit la base SQLite locale (database.db) avec better-sqlite3
 * et seed toutes les données vers la base Turso distante.
 *
 * Pré-requis :
 ...
 *   node migrate-to-turso.js --reset   (vide les tables distantes avant de seeder)
 * -----------------------------------------------------------------
 */
...
// Schéma distant (identique au schéma local)
const SCHEMA = {
...
// Découpe un tableau en morceaux (pour ne pas envoyer un batch() trop gros)
...
async function main() {
  if (!process.env.TURSO_DATABASE_URL || !process.env.TURSO_AUTH_TOKEN) {
    console.error(
      "❌ TURSO_DATABASE_URL et TURSO_AUTH_TOKEN doivent être définis (fichier .env)."
    );
    process.exit(1);
  }
  ...
  // Désactive temporairement les FK côté remote pendant la création/le seed
  await remote.execute("PRAGMA foreign_keys = OFF;");

  // 1. Création du schéma distant
  console.log("🛠️  Création du schéma sur Turso (IF NOT EXISTS)...");
  for (const table of TABLES_IN_ORDER) {
    await remote.execute(SCHEMA[table]);
  }

  // 2. Reset optionnel (utile si tu relances le script plusieurs fois)
  if (RESET) {
    ...
    await remote.execute(
      `DELETE FROM sqlite_sequence WHERE name IN (${TABLES_IN_ORDER.map(
        (t) => `'${t}'`
      ).join(",")});`
    );
  }

  // 3. Seed table par table, dans l'ordre des FK
  for (const table of TABLES_IN_ORDER) {
